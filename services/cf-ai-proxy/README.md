...
Hệ thống Proxy hiệu năng cao viết bằng **Golang (Gin Framework)**, đóng vai trò làm cổng trung gian (Adapter) chuyển đổi các yêu cầu từ AI Agents hoặc API Gateway (như 9router) thành cấu trúc tương thích với **Cloudflare Workers AI**.
...
## ⚙️ Quy Ước Đặt Tên & Tránh Xung Đột Trên 9router

Để tránh xung đột với Cloudflare provider chính thức trực tiếp trên 9router, hệ thống sử dụng quy ước đặt tên riêng biệt (Isolated Namespace Prefix):

### 1. Trên 9router (Cấu hình Provider)
* Khai báo một Custom Provider (OpenAI hoặc Anthropic Compatible) trỏ về proxy của chúng ta:
  * **Base URL**: `http://host.docker.internal:3000/v1` (Nếu 9router chạy Docker) hoặc `http://127.0.0.1:3000/v1` (Chạy trực tiếp trên Host).
...
```
Khi client gọi combo thông qua 9router:
1. 9router sẽ định tuyến theo tiền tố `cf-ai-proxy/` và chuyển tiếp yêu cầu về proxy.
...
3. Cách làm này cô lập hoàn toàn luồng xử lý và không gây xung đột định tuyến với Cloudflare provider trực tiếp của 9router.
